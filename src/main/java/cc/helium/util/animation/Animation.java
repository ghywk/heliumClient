package cc.helium.util.animation;
...
public abstract class Animation {
    ...
    public double getLinearOutput() {
        return 1 - ((timerUtil.getElapsedTime() / (double) duration) * endPoint);
    }
    ...
    public boolean isDone() {
        return timerUtil.finished(duration);
    }
    ...
    public Animation setDirection(Direction direction) {
        if (this.direction != direction) {
            ...
            timerUtil.setMillis(System.currentTimeMillis() - (duration - Math.min(duration, timerUtil.getElapsedTime())));
        }
        return this;
    }
